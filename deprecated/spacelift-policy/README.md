...
- [`http`](https://registry.terraform.io/modules/http/>= 3.0), version: >= 3.0
...
<dl>
  <dt>`policies` (`any`) <i>required</i></dt>
  <dd>
    The map of required policies to add.<br/>
...
<dl>
  ...
  <dd>
    List of global labels to add to each policy. These values can be overridden in `var.policies`'s per policy `labels` key.<br/>
    <br/>
    **Type:** `list(string)`
    <br/>
    **Default value:** `[]`
  </dd>
  <dt>`policy_version` (`string`) <i>optional</i></dt>
  <dd>
    The optional global policy version injected using a %s in each `body_url`. This can be pinned to a version tag or a branch.<br/>
    <br/>
    **Type:** `string`
    <br/>
  ...
  <dd>
    The global `space_id` to assign to each policy. This value can be overridden in `var.policies`'s per policy `space_id` key.<br/>
    <br/>
    **Type:** `string`
    <br/>
    **Default value:** `"root"`
