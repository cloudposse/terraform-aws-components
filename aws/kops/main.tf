...
resource "aws_ssm_parameter" "kops_shared_nat_gateways" {
  ...
  value       = "${var.use_shared_nat_gateways == "true" ? join("", data.aws_ssm_parameter.nat_gateways.*.value) : join(",", formatlist("External", split(",", data.aws_ssm_parameter.private_subnet_ids.*.value)))}"
