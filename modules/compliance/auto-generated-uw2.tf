...
module "compliance_uw2" {
  source = "./modules/single-account"
  providers = {
    aws = aws.uw2
  }

  enabled                            = contains(local.enabled_regions, "us-west-2")
  region                             = "us-west-2"
  ...
  create_iam_role = data.aws_region.this.name == "us-west-2"
  iam_role_arn    = data.aws_region.this.name == "us-west-2" ? null : local.config_iam_role_arn
