...
module "compliance_se1" {
  source = "./modules/single-account"
  providers = {
    aws = aws.se1
  }

  enabled                            = contains(local.enabled_regions, "sa-east-1")
  region                             = "sa-east-1"
  ...
  create_iam_role = data.aws_region.this.name == "sa-east-1"
  iam_role_arn    = data.aws_region.this.name == "sa-east-1" ? null : local.config_iam_role_arn
