...
module "compliance_ue1" {
  source = "./modules/single-account"
  providers = {
    aws = aws.ue1
  }

  enabled                            = contains(local.enabled_regions, "us-east-1")
  region                             = "us-east-1"
  ...
  create_iam_role = data.aws_region.this.name == "us-east-1"
  iam_role_arn    = data.aws_region.this.name == "us-east-1" ? null : local.config_iam_role_arn
