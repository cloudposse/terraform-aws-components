...
module "compliance_as0" {
  source = "./modules/single-account"
  providers = {
    aws = aws.as0
  }

  enabled                            = contains(local.enabled_regions, "ap-south-1")
  region                             = "ap-south-1"
  ...
  create_iam_role = data.aws_region.this.name == "ap-south-1"
  iam_role_arn    = data.aws_region.this.name == "ap-south-1" ? null : local.config_iam_role_arn
