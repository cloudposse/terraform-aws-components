...
module "compliance_uw1" {
  source = "./modules/single-account"
  providers = {
    aws = aws.uw1
  }

  enabled                            = contains(local.enabled_regions, "us-west-1")
  region                             = "us-west-1"
  ...
  create_iam_role = data.aws_region.this.name == "us-west-1"
  iam_role_arn    = data.aws_region.this.name == "us-west-1" ? null : local.config_iam_role_arn
