...
module "compliance_as2" {
  source = "./modules/single-account"
  providers = {
    aws = aws.as2
  }

  enabled                            = contains(local.enabled_regions, "ap-southeast-2")
  region                             = "ap-southeast-2"
  ...
  create_iam_role = data.aws_region.this.name == "ap-southeast-2"
  iam_role_arn    = data.aws_region.this.name == "ap-southeast-2" ? null : local.config_iam_role_arn
