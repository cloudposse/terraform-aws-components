...
module "compliance_ew3" {
  source = "./modules/single-account"
  providers = {
    aws = aws.ew3
  }

  enabled                            = contains(local.enabled_regions, "eu-west-3")
  region                             = "eu-west-3"
  ...
  create_iam_role = data.aws_region.this.name == "eu-west-3"
  iam_role_arn    = data.aws_region.this.name == "eu-west-3" ? null : local.config_iam_role_arn
