...
module "compliance_an1" {
  source = "./modules/single-account"
  providers = {
    aws = aws.an1
  }

  enabled                            = contains(local.enabled_regions, "ap-northeast-1")
  region                             = "ap-northeast-1"
  ...
  create_iam_role = data.aws_region.this.name == "ap-northeast-1"
  iam_role_arn    = data.aws_region.this.name == "ap-northeast-1" ? null : local.config_iam_role_arn
