...
module "compliance_cc1" {
  source = "./modules/single-account"
  providers = {
    aws = aws.cc1
  }

  enabled                            = contains(local.enabled_regions, "ca-central-1")
  region                             = "ca-central-1"
  ...
  create_iam_role = data.aws_region.this.name == "ca-central-1"
  iam_role_arn    = data.aws_region.this.name == "ca-central-1" ? null : local.config_iam_role_arn
