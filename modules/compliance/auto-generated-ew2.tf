...
module "compliance_ew2" {
  source = "./modules/single-account"
  providers = {
    aws = aws.ew2
  }

  enabled                            = contains(local.enabled_regions, "eu-west-2")
  region                             = "eu-west-2"
  ...
  create_iam_role = data.aws_region.this.name == "eu-west-2"
  iam_role_arn    = data.aws_region.this.name == "eu-west-2" ? null : local.config_iam_role_arn
