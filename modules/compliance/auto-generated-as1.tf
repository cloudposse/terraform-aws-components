...
module "compliance_as1" {
  source = "./modules/single-account"
  providers = {
    aws = aws.as1
  }

  enabled                            = contains(local.enabled_regions, "ap-southeast-1")
  region                             = "ap-southeast-1"
  ...
  create_iam_role = data.aws_region.this.name == "ap-southeast-1"
  iam_role_arn    = data.aws_region.this.name == "ap-southeast-1" ? null : local.config_iam_role_arn
