...
module "compliance_ue2" {
  source = "./modules/single-account"
  providers = {
    aws = aws.ue2
  }

  enabled                            = contains(local.enabled_regions, "us-east-2")
  region                             = "us-east-2"
  ...
  create_iam_role = data.aws_region.this.name == "us-east-2"
  iam_role_arn    = data.aws_region.this.name == "us-east-2" ? null : local.config_iam_role_arn
