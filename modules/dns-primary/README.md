...
| domain\_names | Root domain name list | `list(string)` | `null` | no |
...
| record\_config | DNS Record config | <pre>list(object({<br>    root_zone = string<br>    name      = string<br>    type      = string<br>    ttl       = string<br>    records   = list(string)<br>  }))</pre> | `[]` | no |
