...
| <a name="input_alias_record_config"></a> [alias\_record\_config](#input\_alias\_record\_config) | DNS Alias Record config | <pre>list(object({<br>    root_zone     = string<br>    name          = string<br>    type          = string<br>    zone_id       = string<br>    record        = string<br>    target_health = string<br>  }))</pre> | `[]` | no |
