...
Below are examples of Datadog browser and API synthetics tests:
...
The configuration examples are defined in the YAML files in the [catalog/synthetics/examples](catalog/synthetics/examples) folder.
...
### Synthetics Processing Flow

- The component loads the YAML configuration files from the filesystem paths specified by the `synthetics_paths` variable
...
- Then, in the [synthetics](https://github.com/cloudposse/terraform-datadog-platform/blob/master/modules/synthetics/main.tf) module,
  the YAML configuration files are merged and transformed from YAML to
  the [Datadog Terraform provider](https://registry.terraform.io/providers/DataDog/datadog/latest/docs/resources/synthetics_test) format

- And finally, the Datadog Terraform provider converts the synthetic tests into the 
  [Datadog Synthetics API](https://docs.datadoghq.com/api/latest/synthetics) format and provisions them by calling the Datadog API
...
| <a name="module_datadog_synthetics_private_location"></a> [datadog\_synthetics\_private\_location](#module\_datadog\_synthetics\_private\_location) | cloudposse/stack-config/yaml//modules/remote-state | 1.4.1 |
