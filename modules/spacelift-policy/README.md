...
| <a name="input_labels"></a> [labels](#input\_labels) | List of global labels to add to each policy. These values can be overriden in `var.policy`'s per policy `labels` key. | `list(string)` | `[]` | no |
...
| <a name="input_policy"></a> [policy](#input\_policy) | The map of required policies to add. | `map(any)` | n/a | yes |
...
| <a name="input_space_id"></a> [space\_id](#input\_space\_id) | The global `space_id` to assign to each policy. This value can be overriden in `var.policy`'s per policy `space_id` key. | `string` | `null` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_policy"></a> [policy](#output\_policy) | All calculated policies |
