...
| <a name="input_labels"></a> [labels](#input\_labels) | List of global labels to add to each policy. These values can be overriden in `var.policies`'s per policy `labels` key. | `list(string)` | `[]` | no |
...
| <a name="input_space_id"></a> [space\_id](#input\_space\_id) | The global `space_id` to assign to each policy. This value can be overriden in `var.policies`'s per policy `space_id` key. | `string` | `"root"` | no |
