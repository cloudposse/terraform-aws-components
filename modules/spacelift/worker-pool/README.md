...
| <a name="input_iam_context"></a> [iam\_context](#input\_iam\_context) | Context variables for naming IAM resources. Note, changing these will cause<br>a dependency cycle. If you run terraform with `-var enabled=false`, you can<br>safely drop the ASG and the worker pool. If pool is in use, it will not be destroyed. | `any` | `{}` | no |
