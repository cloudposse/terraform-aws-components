...
| <a name="input_custom_policy_enabled"></a> [custom\_policy\_enabled](#input\_custom\_policy\_enabled) | Whether to enable or disable the log delivery policy. Conflicts with `var.log_delivery_policy_enabled` and `var.iam_policy_statements`".<br>Custom policy will be used if set to `true` regardless of other variables. | `bool` | `false` | no |
