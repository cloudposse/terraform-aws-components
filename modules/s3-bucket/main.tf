locals {
  ...
  logging = var.logging != null ? {
    bucket_name = var.logging_bucket_name_rendering_enabled ? format("%s-%s-%s-%s-%s", var.namespace, var.tenant, var.environment, var.stage, var.logging.bucket_name) : var.logging.bucket_name
    prefix      = var.logging_bucket_name_rendering_enabled ? format("%s/%s/", var.logging.prefix, var.name) : var.logging.prefix
  } : null
}
