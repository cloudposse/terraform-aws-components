...
variable "replication_rules" {
  # This shows the schema of the variable `replication_rules`.
  # We made the type `list(any)` because of some issues with the latest versions of Terraform
  # where it added the "strict" typing, which caused problems when the module used in top-level modules.
  # Using list(any) solves the issues.
  # This is what needs to be provided if we want to use `replication_rules`:
  # type = list(object({
  #   id          = string
  #   priority    = number
  #   prefix      = string
