...
| <a name="input_ssm_password_source"></a> [ssm\_password\_source](#input\_ssm\_password\_source) | If var.ssm\_passwords\_enabled is true, DB user passwords will be retrieved from SSM using the key `format(var.ssm_password_source, local.db_username)`. If this value is not set, a default path will be created using the SSM path prefix and ID of the associated Aurora Cluster. | `string` | `""` | no |
| <a name="input_ssm_passwords_enabled"></a> [ssm\_passwords\_enabled](#input\_ssm\_passwords\_enabled) | Whether or not to use SSM for user passwords | `bool` | `true` | no |
