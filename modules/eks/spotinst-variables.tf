...
locals {
  ...
  spotinst_enabled = local.enabled && length(local.ocean_names) > 0
}
...
variable "spotinst_instance_profile_pattern" {
  ...
  description = <<-EOT
    ...
    `format(spotinst_instance_profile_pattern, var.namespace, var.tenant, var.environment, var.stage)`
    If empty or null, a new instance profile will be created.
    EOT
  default     = "%[1]v-gbl-%[4]v-spotinst-worker"
}
