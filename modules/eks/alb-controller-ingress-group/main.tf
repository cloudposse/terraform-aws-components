locals {
  ...
  # for outputs
  annotations           = try(kubernetes_ingress.default[0].metadata.0.annotations, null)
  group_name_annotation = try(lookup(kubernetes_ingress.default[0].metadata.0.annotations, "alb.ingress.kubernetes.io/group-name", null), null)
...
resource "kubernetes_ingress" "default" {
    ...
    annotations = merge(
      {
        ...
        "alb.ingress.kubernetes.io/wafv2-acl-arn"      = module.waf.outputs.acl.arn
  ...
  spec {
    backend {
      service_name = local.default_rule
    ...
    dynamic "rule" {
      ...
      content {
        ...
        http {
          path {
            backend {
              service_name = kubernetes_service.default[0].metadata[0].name
              service_port = var.kubernetes_service_port
            }
