...
$ atmos terraform varfile argocd-deploy-non-prod -s tenant-gbl-corp
$ cd components/terraform/argocd-repo
$ terraform import -var "import_profile_name=eg-tenant-gbl-corp-admin" -var-file="tenant-gbl-corp-argocd-deploy-non-prod.terraform.tfvars.json" "github_repository.default[0]" argocd-deploy-non-prod
$ atmos terraform varfile argocd-deploy-non-prod -s tenant-gbl-corp
$ cd components/terraform/argocd-repo
$ terraform import -var "import_profile_name=eg-tenant-gbl-corp-admin" -var-file="tenant-gbl-corp-argocd-deploy-non-prod.terraform.tfvars.json" "github_branch.default[0]" argocd-deploy-non-prod:main
$ cd components/terraform/argocd-repo
$ terraform import -var "import_profile_name=eg-tenant-gbl-corp-admin" -var-file="tenant-gbl-corp-argocd-deploy-non-prod.terraform.tfvars.json" "github_branch_default.default[0]" argocd-deploy-non-prod
```

The `generate(_config)?_deploy_keys.sh` scripts were previously used to generate keys manually but are deprecated in favor of setting the `deploy_key_generation_enabled` variable to `true` which generates the keys and stores them in SSM automatically.
...
| [github_repository_deploy_key.generated_deploy_keys](https://registry.terraform.io/providers/integrations/github/latest/docs/resources/repository_deploy_key) | resource |
...
| [github_repository_file.config_environment_overlays](https://registry.terraform.io/providers/integrations/github/latest/docs/resources/repository_file) | resource |
| [github_repository_file.config_global_bases](https://registry.terraform.io/providers/integrations/github/latest/docs/resources/repository_file) | resource |
| [github_repository_file.config_tenant_overlays](https://registry.terraform.io/providers/integrations/github/latest/docs/resources/repository_file) | resource |
...
| [aws_ssm_parameter.private_deploy_keys](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/ssm_parameter) | data source |
| [aws_ssm_parameter.public_deploy_keys](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/ssm_parameter) | data source |
...
| <a name="input_applicationset_template"></a> [applicationset\_template](#input\_applicationset\_template) | The name of the applicationset template used to initialize each environment.<br>Valid values are:<br>  apps.applicationset.yaml.tpl (default)<br>  config.applicationset.yaml.tpl | `string` | `"apps.applicationset.yaml.tpl"` | no |
| <a name="input_attributes"></a> [attributes](#input\_attributes) | ID element. Additional attributes (e.g. `workers` or `cluster`) to add to `id`,<br>in the order they appear in the list. New attributes are appended to the<br>end of the list. The elements of the list are joined by the `delimiter`<br>and treated as a single ID element. | `list(string)` | `[]` | no |
| <a name="input_cluster_config_types"></a> [cluster\_config\_types](#input\_cluster\_config\_types) | Cluster configuration types to initialize when `var.applicationset_template == config.applicationset.yaml.tpl` | `set(string)` | `[]` | no |
...
| <a name="input_deploy_key_generation_enabled"></a> [deploy\_key\_generation\_enabled](#input\_deploy\_key\_generation\_enabled) | If true, the private keys for the GitHub Deploy Keys will be created and written to SSM Parameter Store.<br><br>If false, it is expected that the private keys are exist in SSM beforehand. | `bool` | `false` | no |
...
| <a name="input_environments"></a> [environments](#input\_environments) | Environments to populate `applicationset.yaml` files and repository deploy keys (for ArgoCD) for.<br><br>`auto-sync` determines whether or not the ArgoCD application will be automatically synced. | <pre>list(object({<br>    tenant               = string<br>    environment          = string<br>    stage                = string<br>    auto-sync            = bool<br>    auto-sync-namespaces = bool<br>    slack_channel        = string<br>  }))</pre> | `[]` | no |
...
| <a name="input_github_token_override"></a> [github\_token\_override](#input\_github\_token\_override) | TODO | `string` | `null` | no |
