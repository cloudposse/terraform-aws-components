...
locals {
  sops_kubernetes_host                   = data.aws_eks_cluster.kubernetes[0].endpoint
  sops_kubernetes_token                  = data.aws_eks_cluster_auth.kubernetes[0].token
  sops_kubernetes_cluster_ca_certificate = base64decode(data.aws_eks_cluster.kubernetes[0].certificate_authority[0].data)
