...
| <a name="input_additional_targets"></a> [additional\_targets](#input\_additional\_targets) | Additional target routes to add to the ALB that point to this service. The only difference between this and `vanity_alias` is vanity alias will create an alias record in route53 in the hosted zone in this account as well. Additional Targets only adds the listener route to this service's target group. | `list(string)` | `[]` | no |
